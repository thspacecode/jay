...
def get_customer_user(
	*,
	chat_integration: "OmniChannelChatProvider",
	user_id: str,
) -> "User":
	provider = chat_integration.provider
...
def get_raven_channel(
	*,
	chat_integration: "OmniChannelChatProvider",
	raven_user: "RavenUser",
) -> "RavenChannel":
	channel_name = f"{chat_integration.name}_{raven_user.name}"
		...
		channel = frappe.get_doc(
			{
				...
				"omni_channel_chat_provider": chat_integration.name,
				"is_customer": True,
				"enabled": True,
				"workspace": chat_integration.raven_workspace,
			}
...
def handle_incoming_webhook_message(
	*,
	...
	chat_integration = provider.provider_config
